...
class ReadHtml:
    ...
    def getPageHtml(url='https://', main=False):
        ...
        try:
            ...
            ReadHtml.savePage(url, response, main)
            return str(response)
    ...
    def create_folder(folders, main=False):
        first_folder = True
        path_folder_create = '' if main == True else ReadHtml.main_folder
        for folder in folders:
            if main == True:
    ...
    def savePage(url, page, main=False):
        ...
        if main == True:
            ...
            ReadHtml.main_folder = folders[0]
            # concatenar a(s) pasta(s) criadas no caminho do arquivo com o nome do arquivo
            filepath = r'%s/%s.html' % (ReadHtml.main_folder, urlname)
            print("MAIN\n")
        else:
            folders.extend(
            ...
            folders = folders[:-1]
            filepath = r'%s/%s%s.html' % (ReadHtml.main_folder,
                                          ReadHtml.format_path(url, folders), filename)

        print('Pastas = ', folders)
        # variavel para concatenar o caminho de criacao das pastas
        ReadHtml.create_folder(folders, main)
